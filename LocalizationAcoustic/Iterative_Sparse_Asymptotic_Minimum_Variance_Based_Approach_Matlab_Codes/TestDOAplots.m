...
% limited by grids
M         = 12; %10; % # of sensors
t_samples = 120; %120; %16; % 200; % # of snapshots obtained by the ULA
...
% Fixed Source powers
PowerDOAdB = [5; 3]; %[20 15]; %[20; 20; 15]; % in dB
...
ylabel('Power / dB');
if Cohr_flag
	title(['Coherent SAMV-3 ' num2str(SNR) 'dB']);
else
    title(['Independent SAMV-3 ' num2str(SNR) 'dB']);
end
xlim([min(DOAscan) max(DOAscan)]);
myboldify1;
if Flag_save_fig
...
h_r1ML = figure; 
plot(theta_k_mat_r1ML, 10 * log10(p_k_mat_r1ML + eps), 'b*');
hold on;
plot(DOA,  20*log10(amplitudeDOA.'), 'ro');
legend('Estimates', 'Truth');
xlabel('Angle / degree');
ylabel('Power / dB');
if Cohr_flag
	title(['Coherent R1ML ' num2str(SNR) 'dB']);
else
    title(['Independent R1ML ' num2str(SNR) 'dB']);
end
xlim([min(DOAscan) max(DOAscan)]);
myboldify1;
if Flag_save_fig
...
for samk = [  1 3]
    ...
    h_ML = figure; 
    plot(theta_k_mat, 10 * log10(p_k_mat + eps), 'b*');
    hold on;
    plot(DOA,  20*log10(amplitudeDOA.'), 'ro');
    legend('Estimates', 'Truth');
    xlabel('Angle / degree');
    ylabel('Power / dB');
    if Cohr_flag
        title(['Coherent S' num2str(samk) 'ML ' num2str(SNR) 'dB']);
    else
        title(['Independent S' num2str(samk) 'ML ' num2str(SNR) 'dB']);
    end
    xlim([min(DOAscan) max(DOAscan)]);
    myboldify1;
    if Flag_save_fig
...
line(line_x, line_y, 'linewidth', 1, 'color', 'r'  );
if Flag_save_fig
...
% check whether MUSIC is perfect, compare with the R1ML results

AngleSE_R1ML = (theta_k_mat_r1ML - DOA) * (theta_k_mat_r1ML - DOA)';
AngleSE_MUSIC = (theta_k_select_music - DOA) * (theta_k_select_music - DOA)';

disp(['Angle SE of RIML: ' ]);
disp(AngleSE_R1ML);

disp(['Angle SE of MUSIC: ' ]);
disp(AngleSE_MUSIC);
...
% % always normal, no need to check that
% [p_k_mat, sigma_updata, theta_k_mat, Distance, normal]=...
%     fun_R3_slowCorrect(y_noisy,A,DOAscan,DOA); 

[p_k_mat, sigma_updata, theta_k_mat, Distance, normal]=...
    fun_R3(y_noisy,A,DOAscan,DOA); 

time = toc;
disp(['Remark 3 algo running time ==' num2str(time) ' sec.' ]);
...
xlabel('Angle / degree');
ylabel('Power / dB');
title(['Remark3 ' num2str(SNR) 'dB']);
xlim([min(DOAscan) max(DOAscan)]);
myboldify1;
if Flag_save_fig
   saveas(h_R3, [figpath '/R3_DOA_est.fig' ]);
   print('-depsc2',[figpath '/R3_DOA_est.eps']);
end
%}

%{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Sam-k + remark 3 implementation.. 
for samk = [0 1 3]
    tic;

    [p_k_mat, sigma_updata, theta_k_mat, Distance, normal]=...
        fun_SkR3(y_noisy,A,DOAscan,DOA, samk); 

    time = toc;
    disp(['SAM-' num2str(samk) '+Remark 3 algo running time ==' num2str(time) ' sec.' ]);
    ...
    xlabel('Angle / degree');
    ylabel('Power / dB');
    title(['SAM-' num2str(samk) '+Remark3 ' num2str(SNR) 'dB']);
    xlim([min(DOAscan) max(DOAscan)]);
    myboldify1;
    if Flag_save_fig
       saveas(h_R3, [figpath '/S' num2str(samk) 'R3_DOA_est.fig' ]);
       print('-depsc2',[figpath '/S' num2str(samk) 'R3_DOA_est.eps']);
    end

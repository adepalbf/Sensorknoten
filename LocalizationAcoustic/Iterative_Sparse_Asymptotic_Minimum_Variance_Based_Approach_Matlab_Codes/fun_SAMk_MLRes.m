%% =============== Refined: SAMV-k + fminsearch init by SAMV-3 ==========================
function [p_k_mat,  noisePower, theta_k_mat, Distance, normal]=...
    fun_SAMk_MLRes(Y,A,DOA, initPower, initDOA, initNoisePower, normal_in, samk) % OK.
...
for iterIdx = 1:maxIter
    ...
    Rinv = inv(R);
    sigma_updata = real(trace(Rinv*Rinv*R_N) )/real(trace(Rinv*Rinv)); % Samk power-estimates...
...
%         end
    ...
    for source_ind = 1: Numsources
        ...
        %% === start switching sam-k
        switch samk
            case 0 
                p_k = (Detected_powers_current(source_ind)^2) * ...
                    A_current(:, source_ind)' * Rinv * R_N * Rinv * A_current(:, source_ind); % SAMV-0 
            
            case 1
                p_k = A_current(:, source_ind)' * Rinv * R_N * Rinv * A_current(:, source_ind)/...
                    ((A_current(:, source_ind)' * Rinv * A_current(:, source_ind)  )^2); % SAMV-1
            
            case 3
                p_k = Detected_powers_current(source_ind) * ...
                    A_current(:, source_ind)' * Rinv * R_N * Rinv * A_current(:, source_ind)/...
                    (A_current(:, source_ind)' * Rinv * A_current(:, source_ind)  ); % SAMV-3 formula
            otherwise
                disp('Unknown SAMV-k algos!');
        end        
        %% === End switching sam-k
...
%     normal = 0;
%     disp('Samk_ML failed due to Distance > 10! ');
% else
%     normal = 1;
% end
